...
print("=" * 70)
print("BOTTLE IMAGE DOWNLOADER - 2000 IMAGES")
print("Bulletproof with gap filling and duplicate detection")
print("=" * 70)

TARGET_IMAGES = 2000
OUTPUT_FOLDER = "bottle_dataset_2000"
STATE_FILE = "download_state_bottle.json"
...
def scan_existing_images():
    print("\nScanning existing bottle images...")
    existing_indices = set()
    existing_hashes = {}

    if os.path.exists(OUTPUT_FOLDER):
        for filename in os.listdir(OUTPUT_FOLDER):
            if filename.startswith('bottle_') and filename.endswith('.jpg'):
                try:
...
def main():
    ...
    if len(existing_indices) >= TARGET_IMAGES:
        print("\nALREADY HAVE 2000 BOTTLE IMAGES!")
        return
    ...
    while current_target_idx < len(target_indices):
        ...
        for url_idx, url in enumerate(image_urls):
            ...
            img_path = os.path.join(OUTPUT_FOLDER, f"bottle_{target_index:04d}.jpg")
    ...
    if len(final_indices) >= TARGET_IMAGES:
        print("   SUCCESS! 2000 bottle images complete!")
    else:
