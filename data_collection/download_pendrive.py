...
print("=" * 70)
print("PENDRIVE IMAGE DOWNLOADER - 2000 IMAGES")
print("Bulletproof with gap filling and duplicate detection")
print("=" * 70)

TARGET_IMAGES = 2000
OUTPUT_FOLDER = "pendrive_dataset_2000"
STATE_FILE = "download_state_pendrive.json"
...
SEARCH_QUERIES = [
    ...
    "thumb drive",
    "USB drive isolated",
    "pendrive white background",
    "USB flash drive close up",
    "computer USB drive",
    "pendrive technology",
    "USB storage device",
    "pendrive product photo",
    "USB drive black background",
    "colorful pendrives",
    "metal USB drive",
    "plastic USB drive",
    "pendrive side view",
    ...
    "USB drive macro",
    "pendrive collection",
    "modern USB flash drive",
    "pendrive computer accessory",
    "USB drive office supplies",
    "pendrive data storage",
    "USB drive technology",
    "pendrive minimalist",
    "USB drive professional",
    "pendrive corporate",
    "USB flash drive brand",
    "pendrive close up shot",
    "USB drive on desk",
    "pendrive transparent background",
    "USB drive studio shot",
    "pendrive electronic device"
]
...
def scan_existing_images():
    print("\nScanning existing pendrive images...")
    existing_indices = set()
    existing_hashes = {}

    if os.path.exists(OUTPUT_FOLDER):
        for filename in os.listdir(OUTPUT_FOLDER):
            if filename.startswith('pendrive_') and filename.endswith('.jpg'):
                try:
...
def main():
    ...
    if len(existing_indices) >= TARGET_IMAGES:
        print("\nALREADY HAVE 2000 PENDRIVE IMAGES!")
        return
    ...
    while current_target_idx < len(target_indices):
        ...
        for url_idx, url in enumerate(image_urls):
            ...
            img_path = os.path.join(OUTPUT_FOLDER, f"pendrive_{target_index:04d}.jpg")
    ...
    if len(final_indices) >= TARGET_IMAGES:
        print("   SUCCESS! 2000 pendrive images complete!")
    else:
