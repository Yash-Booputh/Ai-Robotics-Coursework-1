...
print("=" * 70)
print("NOTEBOOK IMAGE DOWNLOADER - 2000 IMAGES")
print("Bulletproof with gap filling and duplicate detection")
print("=" * 70)

TARGET_IMAGES = 2000
OUTPUT_FOLDER = "notebook_dataset_2000"
STATE_FILE = "download_state_notebook.json"
...
SEARCH_QUERIES = [
    ...
    "notebook black background",
    "colorful notebooks",
    "notebook stack",
    "notebook collection",
    "notebook paper",
    "notebook cover",
    "notebook product photo",
    "notebook stationery",
...
def scan_existing_images():
    print("\nScanning existing notebook images...")
    existing_indices = set()
    existing_hashes = {}

    if os.path.exists(OUTPUT_FOLDER):
        for filename in os.listdir(OUTPUT_FOLDER):
            if filename.startswith('notebook_') and filename.endswith('.jpg'):
                try:
...
def main():
    ...
    if len(existing_indices) >= TARGET_IMAGES:
        print("\nALREADY HAVE 2000 NOTEBOOK IMAGES!")
        return
    ...
    while current_target_idx < len(target_indices):
        ...
        for url_idx, url in enumerate(image_urls):
            ...
            img_path = os.path.join(OUTPUT_FOLDER, f"notebook_{target_index:04d}.jpg")
    ...
    if len(final_indices) >= TARGET_IMAGES:
        print("   SUCCESS! 2000 notebook images complete!")
    else:
