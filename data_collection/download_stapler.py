...
SEARCH_QUERIES = [
    ...
    "electric stapler",
    "manual stapler",
    "stapler desk organizer",
    "stapler close up",
    "stapler white background",
    "stapler tool",
    "stapler office desk",
    "stapler business",
    "modern stapler",
    ...
    "stapler corporate office",
    "stapler secretary desk",
    "stapler filing documents"
]
...
def scan_existing_images():
    print("\nScanning existing stapler images...")
    existing_indices = set()
    existing_hashes = {}

    if os.path.exists(OUTPUT_FOLDER):
        for filename in os.listdir(OUTPUT_FOLDER):
            if filename.startswith('stapler_') and filename.endswith('.jpg'):
                try:
...
def main():
    ...
    if len(existing_indices) >= TARGET_IMAGES:
        print("\nALREADY HAVE 2000 STAPLER IMAGES!")
        return
    ...
    while current_target_idx < len(target_indices):
        ...
        for url_idx, url in enumerate(image_urls):
            ...
            img_path = os.path.join(OUTPUT_FOLDER, f"stapler_{target_index:04d}.jpg")
    ...
    if len(final_indices) >= TARGET_IMAGES:
        print("   SUCCESS! 2000 stapler images complete!")
    else:
